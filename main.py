...
from client.models import models as client_models
...
from client.handlers import user_handler
from client.schemas import user_schema
...
def get_users(token: Annotated[str, Depends(user_handler.oauth2_scheme)],skip: int = 0, limit: int = 50, db: Session = Depends(get_db)):
...
@app.post("users/register/", response_model = user_schema.User)
def create_user(user: user_schema.UserCreate, db: Session = Depends(get_db)):
    db_user = user_handler.get_user_by_username(db, username=user.username)
    if db_user:
...
def update_user(user_id: int, user_update: user_schema.UserUpdate, db: Session = Depends(get_db)):
    updated_user = user_handler.update_user(db, user_id, user_update)
...
def delete_user(username: str, db: Session = Depends(get_db)):
    if not user_handler.delete_user(db=db, username=username):
        raise HTTPException(status_code=404, detail="User not found")
...
async def login_for_access_token(db: Session = Depends(get_db), form_data: OAuth2PasswordRequestForm = Depends(),
    ...
    access_token_expires = datetime.timedelta(minutes=user_handler.ACCESS_TOKEN_EXPIRE_MINUTES)
    access_token = user_handler.create_access_token(
        data = {"sub": user.username}, expires_delta=access_token_expires
    )
    return user_handler.Token(access_token=access_token, token_type="bearer")
